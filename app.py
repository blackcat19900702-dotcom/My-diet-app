...
with tabs[0]:
    ...
    if c_sel == "Tommi 炭香燒肉米漢堡 (固定數據)":
        ...
        col1, col2, col3 = st.columns(3); b_c = col1.number_input("碳水(g)"); b_p = col2.number_input("蛋白(g)"); b_f = col3.number_input("脂肪(g)")
...
with tabs[1]:
    ...
    if m_opt == "LP33 / AB 優酪乳 (預設)":
        ...
        m_name = st.text_input("輸入名稱 (如：豆漿、燕麥奶)"); m_ml = st.number_input("飲用量 (ml)", value=240.0, key="c_m")
        if st.button("➕ 分析並紀錄"):
...
with tabs[2]:
    ...
    if p_sel == "其他肉類/蛋白質選項":
        p_name = st.text_input("輸入名稱 (如：黑豆、板豆腐)"); p_w = st.number_input("重量 (g)", value=35.0)
...
with tabs[4]:
    ...
    fa = c1.number_input("手動油脂", step=0.5); fr = c2.number_input("水果(份)", step=0.5); sa = c3.number_input("鹽(g)", step=0.5)
...
# --- 6. 結算匯出 (使用可複製的 text_area) ---
st.divider()
st.subheader("📋 Excel 快速複製區")
...
st.text_area("點擊右上方按鈕即可複製：", value=output_str, height=100)
