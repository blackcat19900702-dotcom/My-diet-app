...
# --- 2. 初始化 Session State (確保歷史紀錄與數據同步) ---
if 'daily' not in st.session_state:
    ...
    st.session_state.logs = [] # 儲存明細紀錄

# --- 3. 頁面配置 ---
st.set_page_config(page_title="2710kcal 精準監控", layout="wide")
...
# --- Tab 0: 主食 (含 Tommi 與 外食油脂) ---
with tabs[0]:
    c_sel = st.selectbox("選擇主食", ["白米飯 (60g/份)", "五穀米 (60g/份)", "煮過白麵條 (75g/份)", "Tommi 炭香燒肉米漢堡", "Tommi 壽喜燒肉米漢堡", "自定義/其他"])
    ...
    if "Tommi" in c_sel:
        ...
        if st.button("➕ 紀錄 Tommi"):
            ...
            st.session_state.logs.append(f"{datetime.now().strftime('%H:%M')} - {c_sel} x{num}")
            st.rerun()
    else:
        ...
        if st.button("➕ 紀錄一般主食"):
            ...
            st.session_state.logs.append(f"{datetime.now().strftime('%H:%M')} - {c_sel} {c_w}g")
            st.rerun()

# --- Tab 1: 奶類 (智慧判定) ---
with tabs[1]:
    m_name = st.text_input("輸入飲品名稱 (LP33、豆漿、燕麥奶)")
    ...
    if st.button("➕ 紀錄奶類"):
        ...
        st.session_state.logs.append(f"{datetime.now().strftime('%H:%M')} - {m_name} {m_ml}ml")
        st.rerun()

# --- Tab 2: 肉類 (智慧判定：雞蛋歸類中脂) ---
with tabs[2]:
    p_name = st.selectbox("選擇/輸入蛋白質", ["雞蛋", "雞胸肉", "雞腿肉(去皮)", "鮭魚", "豆腐", "黑豆", "其他"])
    p_w = st.number_input("重量 (g)", value=35.0)
    meth = st.selectbox("烹調方式", ["水煮", "氣炸(+0.5油)", "油炒(+1油)", "油炸(+3.5油)"])
    if st.button("➕ 紀錄蛋白質"):
        serv = p_w / 35
        # 關鍵判別邏輯
        low_keys = ["雞胸", "黑豆", "豆腐", "牛腱", "鯛魚"]
        mid_keys = ["雞蛋", "全蛋", "鮭魚", "雞腿", "梅花"]
        if any(k in p_name for k in low_keys): p_key = "protein_low"
        elif any(k in p_name for k in mid_keys): p_key = "protein_mid"
        else: p_key = "protein_high"
        
        st.session_state.daily[p_key] += serv
        f_map = {"水煮":0, "氣炸":0.5, "油炒":1, "油炸":3.5}
        st.session_state.daily["fat"] += f_map[meth]
        st.session_state.logs.append(f"{datetime.now().strftime('%H:%M')} - {p_name} {p_w}g ({meth})")
        st.rerun()

# --- Tab 3: 蔬菜 (綠色達成率邏輯) ---
with tabs[3]:
    v_name = st.text_input("蔬菜名稱 (綠花椰、高麗菜等)")
    ...
    if st.button("➕ 紀錄蔬菜"):
        ...
        st.session_state.logs.append(f"{datetime.now().strftime('%H:%M')} - {v_name} {v_w}g")
        st.rerun()
...
with tabs[4]:
    ...
    if st.button("➕ 紀錄補充項目"):
        ...
        st.session_state.logs.append(f"{datetime.now().strftime('%H:%M')} - 補充: 油{fa}/果{fr}/鹽{sa}")
        st.rerun()
with tabs[5]:
    w_ml = st.number_input("純水量 (ml)", value=250.0)
    if st.button("➕ 紀錄飲水"):
        st.session_state.water += w_ml
        st.session_state.logs.append(f"{datetime.now().strftime('%H:%M')} - 飲水 {w_ml}ml")
        st.rerun()

# --- 6. 結算匯出區 ---
st.divider()
st.subheader("📋 今日數據匯出 (Excel)")
...
data_list = [
    ...
    f"{st.session_state.daily['veggie']:.1f}", f"{green_achieve:.1f}%", 
...
st.components.v1.html(f"""
    <button onclick="navigator.clipboard.writeText('{copy_data}').then(()=>alert('數據列已複製到剪貼簿！'))" 
