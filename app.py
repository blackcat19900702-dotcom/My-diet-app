...
def init_sheet():
    try:
        ...
        # 請確保你的 Google Sheet 名字真的是 "MyDietLog"
...
if not isinstance(sheet_result, str):
    ...
    if st.sidebar.button("💾 結算並存入 Google"):
        try:
            ...
            st.sidebar.balloons()
            st.sidebar.success("數據已存入 Google！")
        except Exception as e:
...
with c1:
    ...
    st.progress(min(total_kcal / 2710, 1.0) if total_kcal > 0 else 0.0)
with c2:
    st.metric("飲水", f"{st.session_state.water:.0f} / 3000 ml")
    st.progress(min(st.session_state.water / 3000, 1.0) if st.session_state.water > 0 else 0.0)

# 份數監控 (KeyError 修正點：現在 GOALS 與 daily 完全同步)
st.divider()
st.write("### 📊 今日份數監控")
m_cols = st.columns(4)
for i, key in enumerate(GOALS.keys()):
    rem = GOALS[key] - st.session_state.daily[key]
    m_cols[i % 4].metric(key.upper(), f"剩 {rem:.1f}", delta=f"{st.session_state.daily[key]:.1f}")

# --- 5. 輸入區域 ---
st.divider()
tabs = st.tabs(["🍚 澱粉/奶類", "🥩 肉類/蔬果", "🧈 油鹽/飲水"])

with tabs[0]:
    c1, c2 = st.columns(2)
    cw = c1.number_input("熟澱粉重量 (g)", min_value=0.0, step=10.0, key="in_carbs")
    mw = c2.number_input("乳製品份數", min_value=0.0, step=0.5, key="in_milk")
    if st.button("➕ 儲存澱粉/奶類"):
        st.session_state.daily["carbs"] += (cw / 60)
        st.session_state.daily["milk"] += mw
        st.rerun()

with tabs[1]:
    c1, c2 = st.columns(2)
    pw = c1.number_input("肉類重量 (g)", min_value=0.0, step=5.0, key="in_prot")
    vw = c2.number_input("蔬菜重量 (g)", min_value=0.0, step=50.0, key="in_veg")
    if st.button("➕ 儲存肉類/蔬菜"):
...
with tabs[2]:
    c1, c2 = st.columns(2)
    fw = c1.number_input("油脂份數", min_value=0.0, step=0.5, key="in_fat")
    win = c2.number_input("飲水 (ml)", min_value=0.0, step=50.0, value=250.0)
    if st.button("➕ 儲存油脂/飲水"):
        st.session_state.daily["fat"] += fw
        st.session_state.water += win
        st.rerun()

if st.button("🔄 開啟新的一天 (清空當前紀錄)", use_container_width=True):
    st.session_state.daily = {k: 0.0 for k in GOALS.keys()}
    st.session_state.water = 0.0
    st.rerun()
